...
#   $env:FRPCMGR_RELEASE_PROXY_BASES='https://a,https://b'  # 覆盖自建 gh-raw 域名 (逗号分隔)
#   $env:FRPCMGR_INSTALL_PROXY_KEY='frpc-mgr-releases'      # 覆盖 gh-raw 资产配置键 (key)
#   $env:FRPCMGR_DOWNLOAD_PROXY='https://my.mirror/'        # 等价 -Proxy; $env:FRPCMGR_NO_PROXY=1 等价 -NoProxy
# =============================================================================
...
#   frpcmgrd 二进制的配置键 (key) = frpc-mgr-releases; 该通道为首选, 失败后回落 DlProxies + 直连
#   可经环境变量覆盖: FRPCMGR_RELEASE_PROXY_BASES (逗号分隔域名) / FRPCMGR_INSTALL_PROXY_KEY (键)
if ($env:FRPCMGR_RELEASE_PROXY_BASES) {
    $GhRawBases = $env:FRPCMGR_RELEASE_PROXY_BASES -split ',' |
...
$GhRawKey = if ($env:FRPCMGR_INSTALL_PROXY_KEY) { $env:FRPCMGR_INSTALL_PROXY_KEY } else { 'frpc-mgr-releases' }
...
环境变量:
  `$env:FRPCMGR_RELEASE_PROXY_BASES = 'https://a,https://b'  # 覆盖自建 gh-raw 域名 (逗号分隔)
  `$env:FRPCMGR_INSTALL_PROXY_KEY   = 'frpc-mgr-releases'    # 覆盖 gh-raw 资产配置键 (key)
  `$env:FRPCMGR_DOWNLOAD_PROXY      = 'https://my.mirror/'   # 等价 -Proxy
  `$env:FRPCMGR_NO_PROXY            = '1'                    # 等价 -NoProxy
        ...
        foreach ($base in $GhRawBases) {
            ...
            try {
                $rel = Invoke-RestMethod -Uri "$b/$GhRawKey/latest" `
                    -Headers @{ 'User-Agent' = 'frpcmgrd-installer' } -UseBasicParsing -TimeoutSec 15
...
# ----------------------------------------------------------------------------
# 外网 IP 探测 (与 install.ps1 主体同款, 此处独立嵌入让 fmc 自包含)
# ----------------------------------------------------------------------------
$PubIpV4Urls = @(
