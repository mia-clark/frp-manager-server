...
GHRAW_KEY="${FRPCMGR_INSTALL_PROXY_KEY:-frpc-mgr-releases}"
...
环境变量等价形式 (适合 CI/自动化):
  ...
  FRPCMGR_RELEASE_PROXY_BASES=https://a,https://b  # 覆盖自建 gh-raw 域名 (逗号分隔)
  FRPCMGR_INSTALL_PROXY_KEY=frpc-mgr-releases  # 覆盖 gh-raw 资产配置键 (key)
...
download_and_install() {
    ...
    # 回落: 沿用既有 try_download (FRPCMGR_DOWNLOAD_PROXY / DL_PROXIES / GitHub 直连)
    if [ "$_got" != "1" ]; then
