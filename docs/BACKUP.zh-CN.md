...
### 持久化与还原不丢
- channels / schedules / runs 全部存 `meta.json`，与 branding / system_config 同一份原子写文件，**跨重启 / `fmc update` 不丢**。
...
cmd/frpcmgrd/main.go        # 构造 scheduler、AutoStart 后 Start、defer Stop
