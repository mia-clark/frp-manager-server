...
#   对应版本的二进制到 /usr/bin/frpcmgrd。
#
# 参数：
#   --version <x.y.z>   版本号（也可用环境变量 VERSION）。这是 frpcmgrd-fetch
...
# 随包文件
INITD_SRC="$ROOT_DIR/openwrt/files/etc/init.d/frpcmgrd"
CONFIG_SRC="$ROOT_DIR/openwrt/files/etc/config/frpcmgrd"
FETCH_SRC="$ROOT_DIR/openwrt/files/usr/sbin/frpcmgrd-fetch"
...
WORK="$(mktemp -d 2>/dev/null || mktemp -d -t frpcmgrd-ipk)"
cleanup() { [ -n "${WORK:-}" ] && rm -rf "$WORK"; }
trap cleanup EXIT INT TERM

# 生成随包 VERSION 文件（frpcmgrd-fetch 默认拉此版本）
